...
should be read as the following three statements, interpreted elementwise:
...
function ParametricOptimizationProblem(;
    ...
    # Build F = [∇ₓL; g; h].
    F = [Symbolics.gradient(L, x); g; h]

    # Set lower and upper bounds for z.
    z̲ = [-Inf * ones(primal_dimension); -Inf * ones(equality_dimension); zeros(inequality_dimension)]
